...
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
python3 scripts/dev/listening.py validate evals/listening/pilot/experiment.json
python3 scripts/dev/listening.py analyze evals/listening/pilot/experiment.json evals/listening/pilot/synthetic-results.json --out "$tmp/analysis.json" --markdown "$tmp/analysis.md"
