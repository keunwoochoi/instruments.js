...
- Browser audits serve each campaign from its public bundle as the HTTP root and assert that sibling private A/B and ABX analysis keys return 404; omitting roles from public JSON is not treated as sufficient blinding by itself.
...
- Automatic local-storage recovery uses the same validator, removes malformed or contract-invalid records with an actionable warning, continues to a valid older record, and evicts rejected audio-metadata promises so a transient failure can be retried.
