...
- Campaign analysis-key v2 retains each exact reference contract plus candidate manifest/schema and registry/schema digests in private provenance. The public participant experiment remains role-free and contract-free, so exact provenance strengthens analysis without leaking the answer key.
...
Exact-head local validation after the dependency merge: loop suite 71/71; listening Python suite 16/16; frozen hidden-reference/anchor digest unchanged; Chromium and desktop WebKit full A/B, ABX, MUSHRA, recovery, storage-failure, tampered-import, exact-contract campaign, and browser-to-Python round trips; Rust 70/70; TypeScript typecheck/build; full multi-track arrangement benchmark within budget; harness and diff checks. Physical iOS Safari remains external and is not inferred from desktop WebKit.
